...
from ssd.engine.helpers.cudagraph_helpers import (
    ...
    run_ix_tree_decode_cudagraph,
    capture_cudagraph,
    capture_verify_cudagraph,
    capture_fi_tree_decode_cudagraph,
    capture_ix_tree_decode_cudagraph,
...
class ModelRunner:
    ...
    def _init_flashinfer_wrappers(self):
        ...
        if self.config.enforce_eager:
            ...
            if not _HAS_FLASHINFER:
                # Iluvatar: cudagraph async tree-decode via static ixinfer wrapper
                # (IxTreeAttn holds a static float mask buffer; capture_ix_tree_decode
                # captures the draft forward, run_ix_tree_decode copies the per-step
                # mask + inputs and replays).
                from ssd.layers.iluvatar_attn import IxTreeAttn
                mq_len = getattr(self.config, "MQ_LEN", None) or \
                    self.config.async_fan_out * (self.config.speculate_k + 1)
                max_bs = min(self.config.max_num_seqs, 512)
                max_num_blocks = (self.config.max_model_len + self.block_size - 1) // self.block_size
                self.only_prefill_wrapper = IxTreeAttn(
                    max_bs, mq_len, max_num_blocks, self.block_size, self.device)
                return
            max_bs = min(self.config.max_num_seqs, 512)
            max_num_blocks = (self.config.max_model_len + self.block_size - 1) // self.block_size
        ...
        if not self.enforce_eager:
            ...
            if self.config.speculate and self.is_draft and self.config.draft_async:
                if _HAS_FLASHINFER:
                    ...
                    ix_gv, ix_pool, ix_graphs, ix_bs = capture_ix_tree_decode_cudagraph(self)  # Iluvatar static ixinfer tree cudagraph
                    self.graph_vars["ix_tree_decode"] = ix_gv
                    self.graph_pools["ix_tree_decode"] = ix_pool
                    self.graphs["ix_tree_decode"] = ix_graphs
                    self.graph_bs_list["ix_tree_decode"] = ix_bs
    ...
    def allocate_kv_cache(self):
        ...
        for module in self.model.modules():
            if hasattr(module, "k_cache") and hasattr(module, "v_cache"):
                ...
                if self.is_draft and self.draft_async and not self.enforce_eager and _HAS_FLASHINFER:
                    module.prefill_wrappers = self.prefill_wrappers
                elif self.is_draft and self.draft_async:
                    # eager EagerTreeAttn, or non-eager Iluvatar IxTreeAttn
    ...
    def run_model(self, input_ids: torch.Tensor, positions: torch.Tensor, is_prefill: bool, last_only: bool = True, tree_decode_step: int = -1, cache_hits: torch.Tensor | None = None, hidden_states: torch.Tensor | None = None):
        ...
        if is_prefill or self.enforce_eager:
            ...
            return run_ix_tree_decode_cudagraph(self, input_ids, positions, last_only, self.graph_vars["ix_tree_decode"], tree_decode_step, cache_hits, hidden_states=hidden_states)
